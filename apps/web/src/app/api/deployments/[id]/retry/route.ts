...
export async function POST(_req: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!['failed', 'deployment_failed'].includes(deployment.status)) {
            return NextResponse.json(
                { error: `Retry is only allowed for failed deployments. Current status: ${deployment.status}` },
                { status: 400 }
            );
        }

        await updateDeploymentStatus(id, 'queued', { error_message: null });
        await appendDeploymentLog(id, 'Manual retry requested from dashboard', 'INFO');
